# ELEC 3EY4: Autonomous Electric Vehicle Project
...
- **Optimization Algorithms:** Formulated and solved a Quadratic Programming (QP) problem, applying active-set methods (`quadprog` library) to generate optimal control actions in real-time.
- **State Estimation & Sensor Fusion:** Implemented a multi-sensor fusion pipeline, fusing wheel odometry and IMU data using a complementary filter approach (the IMU provides the long-term heading, while wheel odometry provides short-term position).
...
- **Sim-to-Real Transfer:** Gained experience in transitioning algorithms from simulation (Gazebo/F1TENTH) to a physical platform, accounting for real-world factors like noise, latency, and friction.
...
- **Real-Time Systems:** Managed real-time constraints by optimizing critical loops and using timer-based callbacks (`ros::Timer`) to ensure deterministic behavior.

### Data Science & Algorithmic Thinking
- **Data Acquisition & Processing:** Acquired and processed high-frequency sensor data (LiDAR scans at 10Hz, IMU at 100Hz, odometry at 50Hz) in real-time.
...
- **Technical Documentation:** Produced comprehensive technical documentation (this README), clearly describing system architecture, algorithms, and implementation details.
